...
public interface CalculationRepository extends JpaRepository<Calculation, Integer> {
    ...
    @Query(value = "select distinct c.expression from Calculation c inner join c.operations as op where op.type = ?1", nativeQuery = false)
    List<String> findExpressionsWithOperation(@Param("operationId") int operation);

    @Query(value = "select count(distinct c.expression) from Calculation c inner join c.operations as op where op.type = ?1", nativeQuery = false)
